...
*     ..
*     .. External Functions ..
      DOUBLE PRECISION   AR_ZLANHS, AR_DLAMCH
      COMPLEX*16         AR_ZLADIV
      EXTERNAL           AR_ZLANHS, AR_DLAMCH, AR_ZLADIV
*     ..
*     .. External Subroutines ..
      EXTERNAL           ZCOPY, AR_ZLARFG, ZSCAL
*     ..
*     .. Intrinsic Functions ..
...
*
      ...
      SMLNUM = AR_DLAMCH( 'Safe minimum' ) / ULP
*
...
*
         DO 20 K = I, L + 1, -1
            ...
            IF( TST1.EQ.RZERO )
     $         TST1 = AR_ZLANHS( '1', I-L+1, H( L, L ), LDH, RWORK )
...
*           submatrix.
*
*           V(2) is always real before the call to AR_ZLARFG, and hence
*           after the call T2 ( = T1*V(2) ) is also real.
*
            IF( K.GT.M )
     $         CALL ZCOPY( 2, H( K, K-1 ), 1, V, 1 )
            CALL AR_ZLARFG( 2, V( 1 ), V( 2 ), 1, T1 )
            IF( K.GT.M ) THEN
               H( K, K-1 ) = V( 1 )
               H( K+1, K-1 ) = ZERO
            END IF
