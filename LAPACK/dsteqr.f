...
*     .. External Functions ..
      LOGICAL            AR_LSAME
      DOUBLE PRECISION   AR_DLAMCH, AR_DLANST, AR_DLAPY2
...
*
      IF( ICOMPZ.EQ.2 )
     $   CALL AR_DLASET( 'Full', N, N, ZERO, ONE, Z, LDZ )
*
      NMAXIT = N*MAXIT
      JTOT = 0
*
...
*
   60    CONTINUE
     ...
     $      GO TO 80
*
*        If remaining matrix is 2-by-2, use AR_DLAE2 or AR_SLAEV2
...
*
         IF( M.EQ.L+1 ) THEN
            IF( ICOMPZ.GT.0 ) THEN
               ...
               WORK( L ) = C
               WORK( N-1+L ) = S
               CALL AR_DLASR( 'R', 'V', 'B', N, 2, WORK( L ),
...
*
         IF( ICOMPZ.GT.0 ) THEN
            MM = M - L + 1
            CALL AR_DLASR( 'R', 'V', 'B', N, MM, WORK( L ), WORK( N-1+L ),
     $                  Z( 1, L ), LDZ )
         END IF
*
         D( L ) = D( L ) - P
...
*
         IF( M.EQ.L-1 ) THEN
            IF( ICOMPZ.GT.0 ) THEN
               ...
               WORK( M ) = C
               WORK( N-1+M ) = S
               CALL AR_DLASR( 'R', 'V', 'F', N, 2, WORK( M ),
...
*
         IF( ICOMPZ.GT.0 ) THEN
            MM = L - M + 1
            CALL AR_DLASR( 'R', 'V', 'F', N, MM, WORK( M ), WORK( N-1+M ),
     $                  Z( 1, M ), LDZ )
         END IF
*
         D( L ) = D( L ) - P
